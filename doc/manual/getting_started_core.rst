...
.. note:: 
    Only output events are redirected to the DMA core. Input methods inside a ``with dma`` block will be called as they would be outside of the block, in the current real-time context, and input events will be buffered normally, not to DMA. Note however that any advance of ``now`` will be reflected in the recorded scheduling, e.g., the input event may still act as a ``delay()`` of nondeterministic duration in the recording if not compensated for.  
